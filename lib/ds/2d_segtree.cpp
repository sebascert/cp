...
void build() {
	...
	fore(i, 0, n) ford(j, m, 1) st[i + n][j] =
	    op(st[i + n][j << 1], st[i + n][j << 1 | 1]);
	ford(i, n, 1) fore(j, 0, 2 * m) st[i][j] =
	    op(st[i << 1][j], st[i << 1 | 1][j]);
}
void upd(int x, int y, lli v) {
	st[x + n][y + m] = v;
...
lli query(int x0, int x1, int y0, int y1) { //[x0,x1) [y0,y1)
	lli r = NEUT;
	for (int i0 = x0 + n, i1 = x1 + n; i0 < i1; i0 >>= 1, i1 >>= 1) {
		...
		if (i1 & 1)
			t[q++] = --i1;
		fore(k, 0, q) for (int j0 = y0 + m, j1 = y1 + m; j0 < j1;
				   j0 >>= 1, j1 >>= 1) {
			if (j0 & 1)
				r = op(r, st[t[k]][j0++]);
			if (j1 & 1)
